...
## Problem3
Input three datasets. I split the 'baker_name' variable in the 'bakers.csv' because it is the full name of the baker, however the rest two datasets are using the first name of the baker.
```{r}
...
Using anti_join() to check for completeness and correctness across datasets 
```{r}
...
From the results, it is obvious that the baker '\jo\' in bakes.csv and baker.csv

First combine the bakes and corresponding results.
```{r}
results_bakes_df = 
  left_join(results_df, bakes_df, by = c("baker", "series","episode"))

final_data =
  left_join(results_bakes_df,bakers_df, by = c("baker", "series"))
```
